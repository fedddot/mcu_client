...
namespace mcu_client_utl {
	...
	public:
		...
		using UartBaud = typename Uart::UartBaud;

		UartConnection(const std::string& tty_path, const UartBaud& baud, unsigned int timeout_ms, const UartData& head, const UartData& tail);
	...
	private:
		enum : unsigned int { POLLING_TIMEOUT_MS = 100 };
		mutable Uart m_uart;
	...
	inline UartConnection::UartConnection(const std::string& tty_path, const UartBaud& baud, unsigned int timeout_ms, const UartData& head, const UartData& tail): m_uart(tty_path, baud, POLLING_TIMEOUT_MS), m_timeout_ms(timeout_ms), m_head(head), m_tail(tail), m_receiver(head, tail), m_received_data(""), m_is_data_received(false) {
